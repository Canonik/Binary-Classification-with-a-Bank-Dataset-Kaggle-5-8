...
def K_fold_estimation():
    ...
    print(f"Fold AUC scores: {scores}")
...
def Total_dataset_training():
    ...
    joblib.dump(XGB_model, "models/XGB_finetuned_rf_final.pkl")
    ...
    predictions_df.to_csv("reports/XGB_finetuned_rf_final.csv")

def Fine_tuning_pipeline():
    parameters = {
    "xgbclassifier__n_estimators": randint(1400, 4000),      
    "xgbclassifier__learning_rate": uniform(0.005, 0.01),        
    "xgbclassifier__max_depth": randint(11, 15),                   
    ...
    rnd_search = RandomizedSearchCV(XGB_model, parameters, n_iter=1, cv=5, scoring="roc_auc", random_state = 42, verbose=3)
    ...
    useless_oof_pipeline = best_model.named_steps["featureunion"].transformer_list[2][1]

    feature_names = np.concatenate([
        preprocessing.get_feature_names_out(),
        preprocessing_doy.get_feature_names_out(),
        useless_oof_pipeline.get_feature_names_out()
    ])

    sorted_features = sorted(zip(xgb_model.feature_importances_, feature_names), reverse=True)

    for importance, name in sorted_features[:41]:
        print(f"{name}: {importance:.4f}")

    pd.DataFrame(sorted_features, columns = ["importance", "feature"]).to_csv("reports/model_features_rf.csv")
    pd.DataFrame([rnd_search.best_params_]).to_csv("reports/model_parameters_rf.csv")
